...
该方法用于释放 TCP 连接，具体步骤如下：

1. 将 connection 的读写事件从定时器中移除。
1. 将 connection 的读写事件从 epoll 中移除。
1. 将 connection 的读写事件从 posted_accept_events 和 ngx_posted_events 中移除。
1. 调用 `ngx_close_socket` 关闭掉 TCP 连接。
1. 销毁该 ngx_connection_t 对应的 pool。

### ngx_http_free_request

该方法用于释放请求 ngx_http_request_t 的数据结构，该方法并不会去释放连接。具体步骤如下：

1. 调用请求的 cleanup 方法做清理工作。
1. 调用 NGX_HTTP_LOG_PHASE 阶段的回调方法记录日志。
1. 销毁 ngx_http_request_t 中的内存池。

### ngx_http_close_request

该方法用于根据请求的引用计数来决定是否释放请求和关闭连接，如果引用计数大于 0，则仅仅将引用计数减 1，否则会依据 block 标识判断是否释放请求和连接。具体步骤如下：

1. 取出原始请求（`r->main`），将引用计数减 1。
1. 判断引用计数是否为 0，如果大于 0 或者 block 大于 0，则直接返回。
1. 判断引用计数是否为 0，如果为 0 且 block 为 0，则调用：
   - `ngx_http_free_request` 释放请求数据。
   - `ngx_http_close_connection` 释放连接。

### ngx_http_finalize_connection

该方法用于解决 keepalive 和子请求的问题。具体步骤为：

- 如果引用计数为 1，则表示要结束请求，此时需要检查 keepalive 判断结束请求后连接是否复用。
  - 如果 keepalive 为 1，在调用 `ngx_http_set_keepalive`，该方法会设置连接为 keepalive 状态，并且会把请求 ngx_http_request_t 结构体释放，但是不会调用 `ngx_http_close_connection` 关闭连接。
  - 如果 keepalive 为 0，则检查是否需要延迟关闭请求。
    - 如果不需要延迟关闭请求，则直接调用 `ngx_http_close_request` 关闭请求和连接。
    - 如果需要延迟关闭，则调用 `ngx_http_set_lingering_close`，该方法其实就是会把一些事情做完后再关闭连接。
- 如果引用计数不等于 1，检查 discard_body 是否在丢弃包体。
  - 如果 discard_body 为 0，则直接调用 `ngx_http_close_request` 关闭请求和连接。
  - 如果 discard_body 为 1，则表示丢弃包体，设置读事件回调函数为 `ngx_http_discarded_request_body_handler`，并设置超时事件（时间为 `lingering_timeout`），而后调用 `ngx_http_close_request` 关闭请求和连接。

**注意：**

- 引用计数不等于 1 时，虽然都调用了 `ngx_http_close_request`，但是该步骤实质目的时为了将请求引用计数减一。

### ngx_http_terminate_request

该方法用于强制关闭请求，其步骤主要是两个：

1. 设置原始请求（`r->main`）的引用计数为 1。
1. 调用 `ngx_http_close_request` 关闭请求和连接。

### ngx_http_finalize_request

开发 HTTP 模块通常使用 `ngx_http_finalize_request` 来结束请求，其声明时：

```c
void ngx_http_finalize_request(ngx_http_request *r, ngx_int_t rc);
```

- r 是当前请求（可能是子请求）
- rc 可以是 NGX 系统定义的返回值，也可以是 HTTP 响应码，使用不同值时，行为是不一样的。

该方法的主要目的是处理所有可能情况的请求结束，包括处理引用计数、响应发送完毕，异常情况的自动构造响应等。

- 检查 rc 参数，如果为 NGX_DECLINED 则表示请求还需要按照 11 个 HTTP 阶段继续处理下去。
  1. 设置 `r->write_event_handler` 方法为 `ngx_http_core_run_phases`。
  1. 主动调用 `ngx_http_core_run_phases`。
  1. 结束 `ngx_http_finalize_request` 的调用。
- 检查 rc 参数，如果为 NGX_DONE，则表示不需要做任何事，直接调用 `ngx_http_finaliz_connection`，用以释放请求和连接。
- rc 为其他值，则检查当前请求是否为子请求，如果是子请求，则调用 post_handler 进行回调处理，并进行下一步的处理：
  - 再次检查 rc 参数，如果 rc 值为 `NGX_ERROR`、`NGX_HTTP_REQUEST_TIME_OUT`、`NGX_HTTP_CLIENT_CLOSED_REQUES` 等，通过调用 `ngx_http_terminate_request` 进行请求的强制结束。
  - 如果 rc 值为 NGX_HTTP_CREATED、NGX_HTTP_NO_CONTENT 或者 大于等于 NGX_HTTP_SPECIAL_RESPONSE，则表示请求动作是上传文件或是需要构特殊的响应码，这里不做描述。
  - 如果 rc 为其他值，则再次检查当前请求是否为子请求：
    - 对于原始请求，判断缓冲区是否已经发送完毕：
      - 响应过大无法发送完毕，则会设置 `r->write_event_handler` 为 `ngx_http_writer`，并注册给 epoll 和定时器，以便当连接再次可写时，用于发送剩余的数据。
      - 响应已经发送完毕，从定时器移除读写事件，并调用 `ngx_http_finalize_connection`。
    - 对于子请求，将父请求添加到 `posted_postedrequests` 链表中。

**注意：**

- 子请求结束时，要激活父请求继续处理，其方式就是将父请求添加到 `posted_postedrequests` 中，因为在 `ngx_http_run_posted_requests` 会在去调用 `posted_postedrequests` 中的 `write_event_handler`。
