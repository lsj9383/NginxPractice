...
- [Nginx](#nginx)
    ...
    - [http_map 指令](#http_map-指令)
    - [proxy_pass 指令](#proxy_pass-指令)
        - [proxy_pass 的 uri](#proxy_pass-的-uri)
    - [set指令](#set指令)
        - [if语句块的生效顺序问题](#if语句块的生效顺序问题)
    - [Nginx执行阶段](#nginx执行阶段)
        - [十一个处理阶段](#十一个处理阶段)
        - [常用场景](#常用场景)
    - [日志记录](#日志记录)
    - [常用模块](#常用模块)
    - [一些常见的问题](#一些常见的问题)
        - [cookie 丢失问题](#cookie-丢失问题)
        - [`uri`重定向`uri/`的问题](#uri重定向uri的问题)
    - [upstream语句块](#upstream语句块)
        - [主动探测节点存活](#主动探测节点存活)
        - [被动检测节点存活](#被动检测节点存活)
    - [rewrite指令](#rewrite指令)
        - [flag](#flag)
...
- proxy_cache_use_stale，当后端服务器处于指定的状态时，认为服务器不可用，使用历史缓存数据来响应客户端。
  ...
  ```txt
  Syntax: proxy_cache_use_stale error | timeout | invalid_header | updating | http_500 | http_502 | http_503 | http_504 | http_40
...
可以通过 Nginx 启动 HTTP Basic 认证。可以参考 <http://nginx.org/en/docs/http/ngx_http_auth_basic_module.html>。

访问下述 location 配置，将会触发 HTTP Basic 认证。
...
- auth_basic_user_file，指定账号密码文件，文件格式如下：
  ...
  - password 的格式是有限制的，可以使用的类型如下：

    - 使用 `crypt()` 得到的密文。可以通过 `htpasswd` 或 `openssl passwd` 工具获取。
    - 基于 MD5 的密码算法（apr 1），同样可以使用 `htpasswd` 或 `openssl passwd`。
    - [RFC 2307](https://tools.ietf.org/html/rfc2307#section-5.3) 中描述的 `{scheme}data` 语法。
...
使用 htpasswd 工具生成账号密码文件：

```sh
# 指定生成的文件路径和用户名后，输入密码，得到账号密码文件。密码通过密文方式给出。
$ htpasswd -c ${file} ${name}
New password:
